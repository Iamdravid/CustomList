...
namespace CustomList
{
    public class CustomGenericList
    {
        ...
        {
            T[] tempArray = new T[innerArray.Length + 1];
            for (int i = 0; i < innerArray.Length; i++)
            {
                tempArray[i] = innerArray[i];
            }
            tempArray[innerArray.Length] = item;
            innerArray = tempArray;
        }
        public void RemoveAt(T removeItem)
        {
            bool notFound = true;
            T[] tempGenericArray = new T[innerArray.Length - 1];
            for (int i = 0; i < innerArray.Length; i++)
            {
                if (notFound)
                {
                    ...
                    {
                        tempGenericArray[i] = innerArray[i];
                    }
                }
                else
                {
                    tempGenericArray[i - 1] = innerArray[i];
                }
            }
            innerArray = tempGenericArray;
        }
        public string GenericString()
        {
            string genericString = "Digital World: ";
            for (int i = 0; i < innerArray.Length; i++)
            {
